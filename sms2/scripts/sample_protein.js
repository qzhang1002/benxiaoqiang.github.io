...
function sampleProtein (theDocument) {
	...
	if ((checkFormElement (theDocument.forms[0].elements[0]) == false) || (verifyMaxDigits (enteredNumber, maxInput) == false) || (checkFormElement (theDocument.forms[0].elements[1]) == false) || (checkSequenceLength(theDocument.forms[0].elements[1].value, maxInput) == false))	{
		return false;
	}

	var seqNum = parseInt(theDocument.forms[0].elements[5].options[theDocument.forms[0].elements[5].selectedIndex].value);
	...
	for (var i=1; i <= seqNum; i++) {
		outputWindow.document.write ('&gt;' + 'sampled sequence ' + i + ' consisting of ' + enteredNumber + ' residues.\n');
