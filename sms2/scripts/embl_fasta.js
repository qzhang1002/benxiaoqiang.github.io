...
function emblFasta (theDocument) {
	...
	if (testScript() == false) {
    	...
    	if ((checkFormElement (theDocument.forms[0].elements[0]) == false) || (verifyEmbl (theDocument.forms[0].elements[0].value) == false) || (checkTextLength(theDocument.forms[0].elements[0].value, maxInput) == false))	{
		return false;
    	}
...
function emblToFasta (emblFile)	{
	...
	for (var i=1; i < recordArray.length; i++) {
		...
		outputWindow.document.write ("&gt;" + title + addReturns(dnaSequence) + "\n\n");
	}
	return true;
}
