...
function filterProtein (theDocument) {
	...
	if (testScript() == false) {
    	...
    	var re = new RegExp (theDocument.forms[0].elements[4].options[theDocument.forms[0].elements[4].selectedIndex].value,"g");
    	newProtein = theDocument.forms[0].elements[0].value.replace(re,theDocument.forms[0].elements[5].options[theDocument.forms[0].elements[5].selectedIndex].value);
	...
	openPre();
	outputWindow.document.write ('&gt;filtered protein sequence consisting of ' + newProtein.length + ' residues.\n');
