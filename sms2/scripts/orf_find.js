...
function writeOrfs (dnaSequence, geneticCode, startCodons, startPos, strand, theLength)	{
	...
	while (i <= dnaSequence.length - 3)	{
		for (var i = startPos; i <= dnaSequence.length - 3; i = i + 3)	{
			...
			if (((foundStop) && (proteinLength >= theLength)) || ((i >= dnaSequence.length - 5) && (proteinLength >= theLength)))	{
				...
				outputWindow.document.write ('&gt;ORF number ' + (k+1) + ' in reading frame ' + rf + ' on the ' + strand + ' strand extends from base ' + (startPos + 1) + ' to base ' + (i + 3) + '.\n');
				
				outputWindow.document.write (addReturns(sequenceToTranslate) + '\n\n');
				
				outputWindow.document.write ('&gt;Translation of ORF number ' + (k+1) + ' in reading frame ' + rf + ' on the ' + strand + ' strand.\n');				
	...
	if (k == 0)	{
		outputWindow.document.write ('No ORFs were found in reading frame ' + rf + '.\n\n');
	}
	return true;
}
