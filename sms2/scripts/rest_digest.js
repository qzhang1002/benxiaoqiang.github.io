...
function restDigest (theDocument) {	
	...
	for (var i = 0; i < restrictionFragments.length; i++)	{
		...
		restrictionFragments[i].writeFragment(theDocument.forms[0].elements[4].options[theDocument.forms[0].elements[4].selectedIndex].value);
	}
	closePre();
	closeWindow();
	return true;
...
function digest (arrayOfRestrictionFragments, enzyme) {
	...
	var cutDistance = parseInt((enzyme.match(/\)\D*\d+/)).toString().replace(/\)\D*/,""));
	...
	for (var i = 0; i < arrayOfRestrictionFragments.length; i++)	{
		...
		if (arrayOfRestrictionFragments[i].topology == "circular") {
			...
			shiftValue = (arrayOfRestrictionFragments[i].sequence.substring(0, lookAhead)).length;
			var extendedSequence = arrayOfRestrictionFragments[i].sequence.substring(arrayOfRestrictionFragments[i].sequence.length - lookAhead, arrayOfRestrictionFragments[i].sequence.length) + arrayOfRestrictionFragments[i].sequence + arrayOfRestrictionFragments[i].sequence.substring(0, lookAhead);
			...
			while (matchArray = matchExp.exec(arrayOfRestrictionFragments[i].sequence)) {
				matchPosition = matchExp.lastIndex;
				matchPosition = matchPosition - cutDistance;
		...
		if (!(wasCut)) {
			...
			for (var j = 0; j < positions.length; j++)	{
				...
				if (arrayOfRestrictionFragments[i].topology == "circular") {
					...
					startRestrictionFragment = new RestrictionFragment("linear", arrayOfRestrictionFragments[i].sourceName, arrayOfRestrictionFragments[i].sequence.substring(previousCutPosition, positions[j]), previousStartPosition, previousStartPosition + arrayOfRestrictionFragments[i].sequence.substring(previousCutPosition, positions[j]).length - 1, previousEnzyme, enzymeName, arrayOfRestrictionFragments[i].originalLength);
					
				}

				else {
					restrictionFragmentOne = new RestrictionFragment("linear", arrayOfRestrictionFragments[i].sourceName, arrayOfRestrictionFragments[i].sequence.substring(previousCutPosition, positions[j]), previousStartPosition, previousStartPosition + arrayOfRestrictionFragments[i].sequence.substring(previousCutPosition, positions[j]).length - 1, previousEnzyme, enzymeName, arrayOfRestrictionFragments[i].originalLength);
				...
				if (j == (positions.length - 1)) {
					...
					if (startRestrictionFragment == null) {
						restrictionFragmentTwo = new RestrictionFragment("linear", arrayOfRestrictionFragments[i].sourceName, arrayOfRestrictionFragments[i].sequence.substring(positions[j], arrayOfRestrictionFragments[i].sequence.length), previousStartPosition + arrayOfRestrictionFragments[i].sequence.substring(previousCutPosition, positions[j]).length, arrayOfRestrictionFragments[i].stop, enzymeName, arrayOfRestrictionFragments[i].stopEnzyme, arrayOfRestrictionFragments[i].originalLength);
						newFragments.push(restrictionFragmentTwo);
					}
					else {
						restrictionFragmentTwo = new RestrictionFragment("linear", arrayOfRestrictionFragments[i].sourceName, arrayOfRestrictionFragments[i].sequence.substring(positions[j], arrayOfRestrictionFragments[i].sequence.length) + startRestrictionFragment.sequence, previousStartPosition + arrayOfRestrictionFragments[i].sequence.substring(previousCutPosition, positions[j]).length, startRestrictionFragment.stop, enzymeName, startRestrictionFragment.stopEnzyme, arrayOfRestrictionFragments[i].originalLength);
				...
				previousStartPosition = arrayOfRestrictionFragments[i].start + positions[j];
...
function writeFragment(topology) {
	if (this.topology == "linear") {
		outputWindow.document.write ("&gt;" + this.sequence.length + " bp linear fragment from " + topology + " parent " + this.sourceName + ", base " + this.start + " to base " + this.stop + " (" + this.startEnzyme + " - " + this.stopEnzyme + ").\n");
	}
	else {
		outputWindow.document.write ("&gt;" + this.sequence.length + " bp circular molecule from " + topology + " parent " + this.sourceName + ".\n");
	}
...
function RestrictionFragment (topology, sourceName, sequence, start, stop, startEnzyme, stopEnzyme, originalLength) {
