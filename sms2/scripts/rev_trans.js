...
function revTrans (theDocument) {	
	...
	if ((checkFormElement (theDocument.forms[0].elements[0]) == false) || (checkSequenceLength(theDocument.forms[0].elements[0].value, maxInput) == false) || (checkCodonTable (theDocument.forms[0].elements[4].value) == false))	{
		return false;
	}
...
function writeRevTransSeqNoDegen(protein, title, codonTable) {
	...
	outputWindow.document.write ('&gt;' + 'reverse translation of ' + title + ' to a ' + protein.length + ' base sequence of most likely codons.\n');
...
function writeRevTransSeqDegen(protein, title, codonTable) {
	...
	outputWindow.document.write ('&gt;' + 'reverse translation of ' + title + ' to a ' + protein.length + ' base sequence of consensus codons.\n');
	outputWindow.document.write (addReturns(protein));
	outputWindow.document.write ("\n");
	return true;
}
...
function writeRevTransGraph(protein, codonTable) {
	...
	protein = protein.replace(/(.)/g,
                    function (str, p1, offset, s) {
			aminoAcid = codonTable[p1.toString().toLowerCase()];
			return "<b>" + (offset + 1) + "_" + str + "_" + "first</b>\n" + aminoAcid.rulerPosOne + "<b>" + (offset + 1) + "_" + str + "_" + "second</b>\n" + aminoAcid.rulerPosTwo + "<b>" + (offset + 1) + "_" + str + "_" + "third</b>\n" + aminoAcid.rulerPosThree + "\n";                      
                   }
                );
...
function fillRuler() {
	...
	this.rulerPosOne = this.getRuler(this.baseFreqPosOne[0], markG) + 
	...
	this.rulerPosTwo = this.getRuler(this.baseFreqPosTwo[0], markG) + 
	...
	this.rulerPosThree = this.getRuler(this.baseFreqPosThree[0], markG) + 
