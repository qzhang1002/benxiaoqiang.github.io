...
function splitCodons (theDocument) {	
	...
	for (var i = 0; i < arrayOfFasta.length; i++)	{
		...
		outputWindow.document.write ('>' + title + ';codon_positon_1_bases;length=' + position1.length + ';source_length=' + length + '\n' + addReturns(position1) + '\n\n');

		var position2 = getBasesBasedOnCodonPosition(sequence, 2);
		outputWindow.document.write ('>' + title + ';codon_positon_2_bases;length=' + position2.length + ';source_length=' + length + '\n' + addReturns(position2) + '\n\n');

		var position3 = getBasesBasedOnCodonPosition(sequence, 3);
		outputWindow.document.write ('>' + title + ';codon_positon_3_bases;length=' + position3.length + ';source_length=' + length + '\n' + addReturns(position3) + '\n\n');
