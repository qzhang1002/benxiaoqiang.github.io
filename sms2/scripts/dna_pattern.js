...
function writeDnaPattern (dnaSequence, re)	{
	...
	while (matchArray = re.exec(dnaSequence)) {
		...
		outputWindow.document.write ("&gt;match number " + matchCount + " to \"" + simplePattern + "\" start=" + match_start + " end=" + match_end + " on the direct strand\n" + addReturns(matchArray[0]) + "\n\n");
	...
	while (matchArray = re.exec(dnaSequence)) {
		...
		outputWindow.document.write ("&gt;match number " + matchCount + " to \"" + simplePattern + "\" start=" + match_start + " end=" + match_end + " on the reverse strand\n" + addReturns(matchArray[0]) + "\n\n");
		re.lastIndex = re.lastIndex - RegExp.lastMatch.length + 1;			
	}		
